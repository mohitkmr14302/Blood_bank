...
const Post1 = ({ post, ind, settoggle, toggle }) => {
    const deleteblog = async () => {
        swal({
            title: "Are you sure?",
            text: "Once deleted, Your booking cancelled !",
            icon: "warning",
            buttons: true,
            dangerMode: true,
          })
        ...
        return (
            <>
                ...
                <tr style={{ backgroundColor: "#f2f2f2" }}>
                    ...
                    <td className={classes.th}>{post.group}</td>
                    <td className={classes.th}>{new Date(post.date).toDateString()}</td>
                    <td className={classes.th}><button onClick={() => deleteblog()} className={classes.bottom}>Delete</button></td>
                </tr>
